...
public class InventorySearchController extends PanelController<InventorySearchPanel> {
   ...
   private void loadData(DefaultTableModel tableModel, String searchKeyword, String categoryKind,
      ...
      case "대분류":
         ...
         break;
      case "중분류":
         joinSql = "JOIN maincategory m ON p.main_id = m.main_id " + "JOIN mediumcategory mc ON p.medium_id = mc.medium_id ";
         joinSql2 = "AND mc.medium_name LIKE ?";
         break;
      case "소분류":
         joinSql = "JOIN maincategory m ON p.main_id = m.main_id " + "JOIN smallcategory s ON p.small_id = s.small_id ";
...
//   }
//   
   private void loadProductDetails(String product_name) {
       try (Connection connection = DatabaseUtil.getConnection();
               PreparedStatement pstmt = connection.prepareStatement("SELECT * FROM product WHERE product_name = ?")) {
           ...
           try (ResultSet rs = pstmt.executeQuery()) {
               if (rs.next()) {
                   ...
                   MainForm.nav.navigateTo("pdatail", true, product);
               }
           }
       } catch (SQLException e) {
           e.printStackTrace();
